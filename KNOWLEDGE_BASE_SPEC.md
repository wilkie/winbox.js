...
Phase 5 has begun, with the functions outside GDI's text whose behaviour was already recorded: KERNEL's string, memory and profile calls, USER's string calls and `GetSystemMetrics`, and GDI's `GetDeviceCaps` — 24 function pages, all exact, and topics on string collation, global and local memory, profile files and display drivers. Writing them found that `WritePrivateProfileString` trims the caller's own string in place, writes values unquoted and serves a written value from memory until a flush. The `profile` probe now records each of those and the file's bytes after every write, and winbox.js reproduces all of it, the file byte for byte. The line and polygon recordings followed: `MoveTo`, `LineTo` and `Polygon` have pages and a line-drawing topic, and their replays now go through the exported calls rather than straight to the raster. That found `Polygon` was a stub and `LineTo` drew its last pixel; both now match all 10,380 records, the `Polygon` outline newly recorded for the purpose. Routing every text replay through `TextOut` and the calls that set its state found three more: `TextOut` filled the text's box with the selected brush, which a new `textbk` recording with a black brush shows Windows does not; `SetBkMode` sat at an ordinal Windows does not export, so no program reached it; and `SetTextColor` did not reach the colour text is drawn in. With those fixed, `TextOut`, `SetBkMode`, `SetTextAlign` and `SetTextCharacterExtra` are exact, and the export tables disagree with Windows in six places.
