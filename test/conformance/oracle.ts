...
export function renderReport(summaries: OpcodeSummary[], sample: number): string {
  ...
  for (const entry of [...summaries].sort((a, b) => a.passed / a.total - b.passed / b.total)) {
    const percent = entry.total ? ((entry.passed / entry.total) * 100).toFixed(1) : '0.0';
    lines.push(
  ...
  if (failing.length) {
    ...
    for (const entry of failing.sort((a, b) => a.passed / a.total - b.passed / b.total)) {
