...
export async function DialogBox(hinst, lpszDlgTemp, hwndOwner, dlgprc) {
    ...
    if (resourceInfo) {
        ...
        const paddingWidth = (parseInt(computedStyle.paddingLeft) || 0) + (parseInt(computedStyle.paddingRight) || 0);
        const paddingHeight = (parseInt(computedStyle.paddingTop) || 0) + (parseInt(computedStyle.paddingBottom) || 0);
        width -= paddingWidth;
