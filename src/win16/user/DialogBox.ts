...
export async function DialogBox(hinst, lpszDlgTemp, hwndOwner, dlgprc) {
  console.log('DialogBox', hinst, lpszDlgTemp, hwndOwner, dlgprc);
  ...
  const procCS = (dlgprc >> 16) & 0xffff;
  const procIP = dlgprc & 0xffff;
  ...
  if (resourceInfo) {
    ...
    // Read the header
    let position = 0;
    const style = view.getUint32(position, true);
    position += 4;
    ...
    const fonts = this.fonts.lookup('MS Sans Serif');
    const font = fonts.fontFor(8);
    const avgWidth = font.header.dfAvgWidth + 2;
    const avgHeight = font.header.dfPixHeight;
    ...
    // Read menu name
    const ordinal = view.getUint16(position, true);
    position += 2;
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Read class
      const classId = view.getUint8(position);
      position++;
      ...
      // Read extra bytes
      const extra = view.getUint8(position);
      position++;
      ...
      switch (classId) {
        case 0x80: // Button / Frame
          if (style & 0x10000) {
            control = new Button({
              caption: text,
              font: this.fonts.lookup('MS Sans Serif'),
              size: 8,
              weight: 800,
            });
            const hwndControl = 0;
            control.on('click', (e) => {
              ...
              this.scheduler.call(
                ...
                [
                  [hWnd, HWND],
                  [User.WM_COMMAND, UINT],
                  [control.data.id, WPARAM],
                  [(hwndControl << 16) | User.BN_CLICKED, LPARAM],
                ],
                LRESULT
              );
            });
    ...
    // Send WM_INITDIALOG
    await this.scheduler.call(
      User,
      procCS,
      procIP,
      [
        [hWnd, HWND],
        [User.WM_INITDIALOG, UINT],
        [hWnd, WPARAM],
        [0, LPARAM],
      ],
      LRESULT
    ...
    // Show the dialog
    dialog.show();
    const computedStyle = window.getComputedStyle(dialog.element);
    const paddingWidth =
      (parseInt(computedStyle.paddingLeft) || 0) + (parseInt(computedStyle.paddingRight) || 0);
    const paddingHeight =
      (parseInt(computedStyle.paddingTop) || 0) + (parseInt(computedStyle.paddingBottom) || 0);
    width -= paddingWidth;
    ...
    dialog.resize(
      width + (dialog.width - dialog.innerWidth),
      height + (dialog.height - dialog.innerHeight)
    );
    console.log(
      'console',
