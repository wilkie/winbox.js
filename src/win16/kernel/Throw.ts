export function Throw(lpbuf, ret) {
  ...
  const destSegment = (lpbuf >> 16) & 0xffff;
  const destOffset = lpbuf & 0xffff;
  ...
  const callerIP = this.machine.memory.read16(address + 0);
  const callerCS = this.machine.memory.read16(address + 1);
  ...
  this._machine.cpu.core.write16(this._machine.cpu.core.ss, this._machine.cpu.core.sp, callerIP);
  ...
  this._machine.cpu.core.write32(this._machine.cpu.core.ss, this._machine.cpu.core.sp + 4, lpbuf);

  this._machine.cpu.core.write16(this._machine.cpu.core.ss, this._machine.cpu.core.sp + 8, ret);
