...
export class Color {
  ...
  mix(secondary, amount = 0.5) {
    const newR = this.red * (1 - amount) + secondary.red * amount;
    const newG = this.green * (1 - amount) + secondary.green * amount;
    const newB = this.blue * (1 - amount) + secondary.blue * amount;
  ...
  get a8b8g8r8() {
    return (
      ((this._value & 0xff00ff00) | ((this._value >> 16) & 0xff) | ((this._value & 0xff) << 16)) >>>
      0
    );
  }
   ...
   */
  get r8g8b8a8() {
    return (((this._value << 8) & 0xffffff00) | ((this._value >> 24) & 0xff)) >>> 0;
  }
