...
export class Bitmap {
  ...
  declare _view: any;
  declare _width: any;
  declare static ABGR: any;
  declare static ARGB: any;
  declare static BGRA: any;
  declare static OPERATIONS: any;
  declare static RGBA: any;
  /**
   * Creates a Bitmap object with the given properties, palette, and data.
   */
  constructor(width, height, bpp, format, view?, palette?, options: any = {}) {
    this._width = width;
    this._height = height;
    this._bpp = bpp;
    this._format = format;
    ...
    if (this._options.offscreen) {
      ...
      const offscreenData = new Uint8Array(width * height * 4);
      const offscreenView = new DataView(offscreenData.buffer);
  ...
  blit(
    ...
    if (srcData) {
      ...
      if (invert) {
        // FIXME(ts-migration): referenced an undeclared `s`; the only
        // source value in this branch is `clr`.
        clr = ~clr;
        invert = false;
      }
    }
    ...
    for (let y = 0; y < srcHeight; y++) {
      ...
      for (let dX = destX, sX = srcX; dX < destX + srcWidth && dX < this.width; dX++, sX++) {
        ...
        if (source instanceof Color) {
          ...
          const offset = srcOffset + sX;
          if (offset >= 0) {
            s = source.view.getUint8(offset);
          }
        } else if (srcBPP == 32) {
          const offset = srcOffset + sX * 4;
          if (offset >= 0) {
        ...
        if (this.bpp == 1) {
          ...
          const offset = destOffset + ((dX / 8) >>> 0);
  ...
  convert(bpp, palette?, format?) {
    ...
    const realPalette = new Palette(palette);

    let targetbpRow = bpp * width;
    targetbpRow = (targetbpRow + (8 - 1)) & ~(8 - 1);
    const targetWidthBytes = ((targetbpRow >> 3) + (4 - 1)) & ~(4 - 1);

    const bitmapData = new Uint8Array(targetWidthBytes * height);
        ...
        pixels.forEach((pixel) => {
          ...
          if (x < width) {
            if (bpp == 1) {
              ...
              const color = new Color(pixel >> 8);
              const index = realPalette.nearestIndex(color.red, color.green, color.blue);
