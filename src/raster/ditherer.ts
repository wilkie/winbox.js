...
export class Ditherer {
  ...
  fill(context, x, y, w, h, color) {
    ...
    // Get data
    const imageData = context.getImageData(x, y, w * pixel, h * pixel);
    ...
    const table = Ditherer.BAYER8;
    const stride = table[0].length;
    const divisor = Math.pow(stride, 2);
    ...
    for (let j = 0; j < h; j++) {
      for (let i = 0; i < w; i++) {
        ...
        if (color == 0xc0c0c0) {
          ...
          const threshold = table[j % stride][i % stride] + 1;
          ...
          const newR = levelR * 255.0;
          const newG = levelG * 255.0;
          const newB = levelB * 255.0;

          newColor = new Color(newR, newG, newB);
        }

        const position = j * w * pixel * pixel * 4 + i * pixel * 4;
        ...
        for (let m = 0; m < pixel; m++) {
          for (let n = 0; n < pixel; n++) {
            const relative = w * pixel * 4 * m + 4 * n;
  ...
  nearestColors(color, count = 1) {
    ...
    this._palette.forEach((target) => {
      ...
      let distance = Math.pow(r1 - r2, 2) + Math.pow(g1 - g2, 2) + Math.pow(b1 - b2, 2);
  ...
  _generatePalette256() {
    ...
    const ret = Palette.PALETTE16.concat([]);

    // Generate grayscale entries
    const range = [
      0x00, 0x10, 0x20, 0x35, 0x45, 0x55, 0x65, 0x75, 0x8a, 0x9a, 0xaa, 0xba, 0xca, 0xdf, 0xef,
      0xff,
    ];
    range.forEach((value) => {
    ...
    const addRun = (start, ch, lo, melo, me, mehi, hi) => {
      ...
      ret.push([(r << 2) | (r >> 4), (g << 2) | (g >> 4), (b << 2) | (b >> 4)]);
    ...
    const addCycle = (lo, melo, me, mehi, hi) => {
      // Starting at blue
      let hue = 1;
