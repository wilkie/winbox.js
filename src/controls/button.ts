  ...
  set image(value) {
    ...
    if (!this._pictureBox) {
      ...
      this._pictureBox.on('resize', () => {
        ...
        this._label.resize(
          this._pictureBox.width,
          0,
          this.width - this._pictureBox.width - 8,
          this.height
        );

        if (this._label.caption === '') {
          this._pictureBox.move(
            (this.width - this._pictureBox.width) / 2,
            (this.height - this._pictureBox.height) / 2
          );
        }
      });
    }
    ...
    const resize = (event) => {
      if (this._label) {
        this._label.move(
          ...
          (this.height - this._label.height) / 2
        );
      }
    };
