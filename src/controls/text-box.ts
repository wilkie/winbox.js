...
export class TextBox extends Window {
  ...
  initialize() {
    ...
    this._input.addEventListener('keydown', (event) => {
      ...
      ['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'End', 'Home'].forEach((code) => {
        if (event.code === code) {
          event.preventDefault();
        }
      });
  ...
  _mouseDownEvent(event) {
    ...
    // Do reselection.
    const position = this._charPositionAt(event.x);
  ...
  _charPositionAt(x) {
    ...
    if (this._font) {
      ...
      for (let i = 0; i < this.value.length; i++) {
        const current = this.value.slice(i, i + 1);
        const weight = this.options.weight;
        const metrics = this._font.measure(current, {
          weight: weight,
        });
  ...
  _repositionCaret(index) {
    ...
    if (this._font) {
      // Raster font measurement
      const weight = this.options.weight;
      const current = this.value.slice(0, index);
      const metrics = this._font.measure(current, {
        weight: weight,
      });
      x += metrics.width + 1;
  ...
  set color(color) {
    ...
    // Re-run the value setter so the text redraws.
  ...
  set value(value) {
    ...
    if (this._options.font) {
      if (this._options.font instanceof BitmapFont) {
        ...
        this._captionImage.setAttribute(
          'src',
          font.dataFor(this.value, {
