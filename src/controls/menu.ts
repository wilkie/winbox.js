...
export class Menu extends Window {
  ...
  initialize() {
    ...
    const subWindow = new SubMenu({ caption: 'SUBMENU-' + this.options.caption });
    ...
    this._button.on('client-mousedown', (event) => {
      ...
      if (current) {
        ...
        this.root._draggablePlane.mask(box, current.moveDragEndEvent.bind(current));
      }
    });
  ...
  /* FIXME(ts-migration): nothing calls this. Window dispatches client
   * events through `_clientEvent`, so this override never runs. */
