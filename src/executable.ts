...
export class Executable {
    ...
    declare _peOptionalHeader: any;
    declare _resources: any;
    declare _stream: any;
    declare _view: any;
    declare peHeaderOffset: any;
    declare static MACHINES: any;
    declare static MZ: any;
    declare static NE: any;
    declare static NEResource: any;
    declare static NEResourceType: any;
    declare static PE: any;
    declare static PEOptional: any;
    declare static PESection: any;
    declare static RESOURCES: any;
    declare static SYSTEMS: any;
    declare static TYPES: any;
    /**
    ...
    async _readEntryPoints() {
        const start = this.headerOffset + this.neHeader.entryTableOffset;
        const last = this.neHeader.entryTableLength;

        const data = await this._stream.read(start, last);
        ...
        while (offset < last) {
            ...
            const bundleSize = view.getUint8(offset);
            offset++;
            ...
            const segment = view.getUint8(offset);
            offset++;
            ...
            for (let bi = 0; bi < bundleSize; bi++) {
                if (segment == 0xff) {
                    ...
                    // A segment number
                    const segmentNumber = view.getUint8(offset);
                    offset++;

                    // A segment offset
                    const segmentOffset = view.getUint16(offset, true);
                    offset += 2;
    ...
    async _readResourceTable() {
        ...
        if (this.type == Executable.TYPES.NE) {
            ...
            const last = this.headerOffset + this.neHeader.residentNamesOffset;

            const shift = this._view.getUint16(offset, true);
            offset += 2;
            ...
            while (offset < last && limit <= 30) {
                ...
                const resourceType = Util.readStructure(this._view, Executable.NEResourceType, offset, true);

                if (resourceType.id == 0x0) {
                    break;
                }
                else if (resourceType.id < 0x8000) {
                    const subOffset = startingOffset + resourceType.id;
                    const length = this._view.getUint8(subOffset);
                ...
                for (let i = 0; i < resourceType.number; i++) {
                    const resource = Util.readStructure(this._view, Executable.NEResource, offset, true);

                    if (resource.id < 0x8000) {
                        const subOffset = startingOffset + resource.id;
                        const length = this._view.getUint8(subOffset);
        ...
        if (this._nameTable) {
            ...
            for (let i = 0; i < ret.length; i++) {
                ...
                const names = this._nameTable[resourceType.id];
    ...
    async _readNameTable(resource) {
        ...
        const view = new DataView(await this.readResource(resource));
    ...
    async parse() {
        ...
        const resourceMax = this.headerOffset + this.neHeader.residentNamesOffset;
        ...
        const max = Math.max(resourceMax, entryPointMax);
